<template>
	<view class="uni-group" :class="['uni-group--'+mode ,margin?'group-margin':'']" :style="{marginTop: `${top}px` }">
		<slot name="title">
			<view v-if="title" class="uni-group__title" :style="woStyleTitle" luk-comment="add woStyleTitle">
				<text class="uni-group__title-text">{{ title }}</text>
			</view>
		</slot>
		<view class="uni-group__content" :class="{'group-conent-padding':border}" :style="woStyleContent" luk-comment="add woStyleContent">
			<slot />
		</view>
	</view>
</template>
